#include "platform_config.h"

#ifdef CFG_FLIP_BROSE

#include "flip_brose/flip_brose.h"

// 4x6 font, 32 char offset
const uint8_t font_4x6[96][4] =
{
    {0x00, 0x00, 0x00, 0x00}, // SPACE

    {0x00, 0x00, 0x00, 0x4F}, // !
    {0x00, 0x00, 0x07, 0x00}, // "
    {0x00, 0x14, 0x7F, 0x14}, // #
    {0x00, 0x24, 0x2A, 0x7F}, // $
    {0x00, 0x23, 0x13, 0x08}, // %
    {0x00, 0x36, 0x49, 0x55}, // &
    {0x00, 0x00, 0x05, 0x03}, // '
    {0x00, 0x00, 0x1C, 0x22}, // (
    {0x00, 0x00, 0x41, 0x22}, // )
    {0x00, 0x14, 0x08, 0x3E}, // *
    {0x00, 0x08, 0x08, 0x3E}, // +
    {0x00, 0x00, 0x50, 0x30}, // ,
    {0x00, 0x08, 0x08, 0x08}, // -
    {0x00, 0x00, 0x60, 0x60}, // .
    {0x00, 0x20, 0x10, 0x08}, // /

    {0x0E, 0x15, 0x0E, 0x00}, // 0
    {0x12, 0x1F, 0x10, 0x00}, // 1
    {0x12, 0x19, 0x16, 0x00}, // 2
    {0x11, 0x15, 0x1B, 0x00}, // 3
    {0x07, 0x04, 0x1F, 0x00}, // 4
    {0x1F, 0x15, 0x18, 0x00}, // 5
    {0x0E, 0x15, 0x09, 0x00}, // 6
    {0x19, 0x05, 0x03, 0x00}, // 7
    {0x1A, 0x15, 0x0B, 0x00}, // 8
    {0x12, 0x15, 0x0E, 0x00}, // 9

    {0x00, 0x12, 0x00, 0x00}, // :
    {0x20, 0x12, 0x00, 0x00}, // ;
    {0x04, 0x0A, 0x11, 0x00}, // <
    {0x0A, 0x0A, 0x0A, 0x00}, // =
    {0x11, 0x0A, 0x04, 0x00}, // >
    {0x00, 0x00, 0x00, 0x00}, // ?
    {0x00, 0x00, 0x00, 0x00}, // @

    {0x1E, 0x05, 0x1E, 0x00}, // A
    {0x1F, 0x15, 0x0A, 0x00}, // B
    {0x0E, 0x11, 0x0A, 0x00}, // C
    {0x1F, 0x11, 0x0A, 0x00}, // D
    {0x1F, 0x15, 0x11, 0x00}, // E
    {0x1F, 0x05, 0x01, 0x00}, // F
    {0x0E, 0x11, 0x1D, 0x00}, // G
    {0x1F, 0x04, 0x1F, 0x00}, // H
    {0x11, 0x1F, 0x11, 0x00}, // I
    {0x08, 0x10, 0x0F, 0x00}, // J
    {0x1F, 0x04, 0x1B, 0x00}, // K
    {0x1F, 0x10, 0x10, 0x00}, // L
    {0x1F, 0x06, 0x1F, 0x00}, // M
    {0x1E, 0x04, 0x0F, 0x00}, // N
    {0x0E, 0x11, 0x0E, 0x00}, // O
    {0x1F, 0x05, 0x02, 0x00}, // P
    {0x0E, 0x11, 0x0E, 0x20}, // Q
    {0x1F, 0x05, 0x1A, 0x00}, // R
    {0x12, 0x15, 0x09, 0x00}, // S
    {0x01, 0x1F, 0x01, 0x00}, // T
    {0x1F, 0x10, 0x1F, 0x00}, // U
    {0x0F, 0x18, 0x0F, 0x00}, // V
    {0x1F, 0x0C, 0x1F, 0x00}, // W
    {0x1B, 0x05, 0x1B, 0x00}, // X
    {0x03, 0x1C, 0x0E, 0x00}, // Y
    {0x19, 0x15, 0x13, 0x00}, // Z

    {0x00, 0x1F, 0x11, 0x00}, // [
    {0x03, 0x04, 0x18, 0x00}, // backslash
    {0x11, 0x1F, 0x00, 0x00}, // ]
    {0x02, 0x01, 0x02, 0x00}, // ^
    {0x20, 0x20, 0x20, 0x00}, // _
    {0x00, 0x01, 0x02, 0x00}, // `

    {0x00, 0x00, 0x00, 0x00}, // a
    {0x00, 0x00, 0x00, 0x00}, // b
    {0x00, 0x00, 0x00, 0x00}, // c
    {0x00, 0x00, 0x00, 0x00}, // d
    {0x00, 0x00, 0x00, 0x00}, // e
    {0x00, 0x00, 0x00, 0x00}, // f
    {0x00, 0x00, 0x00, 0x00}, // g
    {0x00, 0x00, 0x00, 0x00}, // h
    {0x00, 0x00, 0x00, 0x00}, // i
    {0x00, 0x00, 0x00, 0x00}, // j
    {0x00, 0x00, 0x00, 0x00}, // k
    {0x00, 0x00, 0x00, 0x00}, // l
    {0x00, 0x00, 0x00, 0x00}, // m
    {0x00, 0x00, 0x00, 0x00}, // n
    {0x00, 0x00, 0x00, 0x00}, // o
    {0x00, 0x00, 0x00, 0x00}, // p
    {0x00, 0x00, 0x00, 0x00}, // q
    {0x00, 0x00, 0x00, 0x00}, // r
    {0x00, 0x00, 0x00, 0x00}, // s
    {0x00, 0x00, 0x00, 0x00}, // t
    {0x00, 0x00, 0x00, 0x00}, // u
    {0x00, 0x00, 0x00, 0x00}, // v
    {0x00, 0x00, 0x00, 0x00}, // w
    {0x00, 0x00, 0x00, 0x00}, // x
    {0x00, 0x00, 0x00, 0x00}, // y
    {0x00, 0x00, 0x00, 0x00}, // z

    {0x00, 0x00, 0x00, 0x00}, // {
    {0x00, 0x00, 0x00, 0x00}, // |
    {0x00, 0x00, 0x00, 0x00}, // }
    {0x00, 0x00, 0x00, 0x00}, // ~

    {0x00, 0x00, 0x00, 0x00}
};

#endif // CFG_FLIP_BROSE
